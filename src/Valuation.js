...
export default class Valuation extends Component {
    ...
    componentDidMount() {
        const { state: { params: { avatar, skill, city, district, start, end, description, bUid, sUid, key, price }, goBack } } = this.props.navigation;
        this.setState({ avatar, skill, city, district, start, end, description, bUid, sUid, key, price, })//上頁傳來是買方的
        //抓賣方名稱
        ...
        //抓出changeButton
        firebase.database().ref('Notification/' + sUid + '/' + key).on('value', (snapshot) => {
            let data = snapshot.val();
            let changeButton = data.changeButton;
                    ...
                    <View style={styles.container}>
                        ...
                        {this.state.changeButton ?
                            ...
                            <View style={{ flex: 1, justifyContent: 'center' }}>
                                <TouchableOpacity
                                    ...
                                    onPress={() => {
                                        ...
                                        //changeButton=false存到賣方資料底下（因為賣方式抓賣方的）
                                        firebase.database().ref('Notification/' + this.state.sUid + '/' + this.state.key).update({
                                            changeButton: false,//是否已搶過
                                        });
                            ...
                            <View style={{ paddingTop: 10, alignItems: 'center' }}>
                                <Text style={{ fontSize: 20 }}>您已搶賺，等待對方反選</Text>
                            </View>
                        }
                    </View>
